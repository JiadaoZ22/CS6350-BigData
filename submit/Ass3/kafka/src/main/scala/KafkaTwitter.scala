...
object KafkaTwitter extends App {
  ...
  if (args.length < 1) {
    println("Correct usage: 0.Topics you search ... ")
    System.exit(1)
  }
...
//  val englishTweet = stream.filter(_.getLang() == "en").map(status => status.getText().
//    replaceAll("(\\b\\w*RT)|[^a-zA-Z0-9\\s\\.\\,\\!,\\@]", "").replaceAll("(http\\S+)","").replaceAll("(@\\w+)","Foo").replaceAll("^(Foo)",""))
  val statuses = englishTweet.map(status => (SentimentAnalyzer.mainSentiment(status.getText())))
...
//    println(rdd.collect())
    ...
    val producer = new KafkaProducer[String, String](props)
      partitionIter.foreach { elem =>
        ...
        val data = new ProducerRecord[String, String]("twitter", null, dat) // first string "instream" is the name of Kafka topic
        producer.send(data)
      }
      producer.flush()
      producer.close()
